...
void Set_System(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin   = USB_DISCONNECT_PIN;          // HJI
...
#else
    RCC_APB2PeriphClockCmd(USB_DISCONNECT_CLK, ENABLE); // HJI
    ...
    GPIO_Init(USB_DISCONNECT_PORT, &GPIO_InitStructure);                // HJI

    GPIO_ResetBits(USB_DISCONNECT_PORT, USB_DISCONNECT_PIN);                   // HJI

    delay(200);                                           // HJI

    GPIO_SetBits(USB_DISCONNECT_PORT, USB_DISCONNECT_PIN);                     // HJI
    ...
    GPIO_Init(USB_DM_PORT, &GPIO_InitStructure);

    /*SET PA11,12 for USB: USB_DM,DP*/
    GPIO_PinAFConfig(USB_DM_PORT, USB_DP_SOURCE, GPIO_AF_14);
    GPIO_PinAFConfig(USB_DM_PORT, USB_DM_SOURCE, GPIO_AF_14);
